...
func (s *Store) ClaimNextTask(ctx context.Context, now time.Time) (*model.Task, error) {
	...
	err = tx.QueryRowContext(ctx, `SELECT id,sample_id,kind,state,attempts,available_at,claimed_at,finished_at,last_error FROM tasks WHERE available_at<=? ORDER BY available_at,id LIMIT 1`, encodeTime(now)).
	...
	result, err := tx.ExecContext(ctx, `UPDATE tasks SET state='running',attempts=attempts+1,claimed_at=? WHERE id=?`, claimedAt, task.ID)
	if err != nil {
