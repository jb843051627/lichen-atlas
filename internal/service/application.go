...
func (a *Application) BuildSiteReport(ctx context.Context, siteID string) (model.SiteReport, error) {
	site, err := a.store.GetSite(context.Background(), siteID)
	if err != nil {
		return model.SiteReport{}, fmt.Errorf("load report site: %w", err)
	}
	samples, err := a.store.ListSamplesBySite(context.Background(), siteID)
	if err != nil {
		return model.SiteReport{}, err
	}
	report := model.SiteReport{Site: *site, GeneratedAt: a.Now()}
	for _, sample := range samples {
		readings, err := a.store.ListReadings(context.Background(), sample.ID)
		if err != nil {
			return model.SiteReport{}, err
		}
		summary := summarizeReadings(readings)
		identification, identErr := a.store.GetIdentification(context.Background(), sample.ID)
		if identErr != nil && !errors.Is(identErr, store.ErrNotFound) {
			return model.SiteReport{}, identErr
		}
		reviews, err := a.store.ListReviews(context.Background(), sample.ID)
		if err != nil {
			return model.SiteReport{}, err
		}
		archive, archiveErr := a.store.GetArchive(context.Background(), sample.ID)
